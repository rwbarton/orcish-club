...
Our 2\nt\ gadget, described in Figure \ref{fig:1M-2NT}, handles both
invitational 4-card raises and slammish 3\+-card raises.  With the
latter hand type, responder may also begin with 2\c\ if balanced.
2\c\ is often preferable if a side 4-4 fit may be better or if a
cue-bidding auction is necessary, while 2\nt\ allows responder to
determine opener's strength and shortness (if any).  It is rather more
difficult to stop in 3\nt\ after choosing 2\nt.
...
After 1\M--2\M, we play two-way game tries.  The next step asks for
the lowest long-suit game try responder would accept; a rebid by
opener then shows a long-suit try in that suit.  The three other bids
below 3\M\ show short-suit game tries (2\nt\ serving as a short-suit
game try in \s\ when \M=\h).  The reraise to 3\M\ is a general
balanced game try, 3\nt\ is natural and nonforcing, and jump shifts
are natural slam tries with extreme two-suiters.

\section{1\h--1\s}\label{1h-1s}

\section{Forcing 1NT}

\section{2/1 GF}\label{2over1}

As previously mentioned 2\d\ and 2\h\ (over 1\s) guarantee 5\+-card
suits while 2\c\ shows either 5\+ clubs or a balanced hand.  The
continuations after 2\c\ are mildly artificial, and we describe them
last.

Opener's rebid is natural, with 2\M\ as a ``catchall''.  Bids above
2\M\ show extras, \emph{except} raising 2\h\ to 3\h, which may be done
on any 3-card fit.  (We have frivolous 3\nt\ available to sort out
opener's range.)  Raising 2\d\ to 3\d\ shows extras although these may
be in the form of extra shape (e.g., any 5-card support).  A jump
shift is a splinter raise of responder's suit, while a non-jump new
suit above 2\M\ shows a 5-card suit and extras.  Jumping to 3\M\ shows
a maximum with a strong suit (at most one loser opposite a singleton).
2\nt\ is a balanced maximum, usually with stoppers in the unbid suits.
2\h\ over 2\d\ just shows 4\+ hearts and 2\M\ (probably the most
common rebid) handles the remaining hands.

On the second round responder may
\begin{itemize}
\item raise (one of) opener's suit(s) naturally (XXX even at the two
  level---does this really need to show a fit?);
\item jump raise (one of) opener's suit(s), showing a picture raise
  (no control in a side suit);
\item jump shift, as a splinter raise for opener's most recent suit
  (usually showing an 8-card fit);
\item bid 2\nt\ if available, tending to deny extra length in any bid
  suits and to show stoppers in the unbid suit(s), with either at most
  17 or more than 20 \hcp;
\item jump to 3\nt, showing the above hand types with about 18--20 \hcp;
\item bid a third suit (semi)naturally, if two suits have been bid;
\item bid the fourth suit artificially, if three suits have been bid
  naturally, typically lacking a stopper in the fourth suit, but
  possibly looking for a fit there;
\end{itemize}
...
We open 1\nt\ with 4333, 4432, 5m332, 5m422 and some 6m322 and 5M332 shapes, when the long suit is poor and the hand has a lot of values in the side suits.
...
&& d) 5-4 or 5-5 in the minors, any strength \\
2\d & Transfer to hearts & a) signoff in hearts \\
&(2\s\ superaccept possible)& b) exactly four hearts or at least 4-4 in the majors, \inv\+ \\
&& c) slammish with long, weak hearts \\
2\h & Transfer to spades & a) signoff in spades \\
&(2\nt\ superaccept possible)& b) exactly four spades, \inv\+ \\
&& c) slammish with long, weak spades \\
2\s & Asks strength & a) balanced and invitational with no four-card major \\
&(2\nt\ = min, 3\c\ = max)& b) balanced, slammish (CONFIT) \\
&& c) weak or \gf\+\ with clubs \\
...
3\c & Transfer to 3\d & weak or \gf\+\ with diamonds \\
3\d & Multi-invitational & long major with game-invitational values \\
3\h & Splinter & 31(54), game forcing \\
3\s & Splinter & 13(54), game forcing \\
3\nt & To play \\
4\c & Gerber \\
...
We play a slightly modified version of Heeman (\cite{Heeman}).  In
general, when responder has shown two suits, we play Kickback 6-key
RKC as the first step above four of responder's higher suit.

\section{1\nt--2\c: \d\ weak or 5\+ major \inv\+ or both minors}

Opener is forced to bid 2\d, then responder can pass or bid:

\begin{itemize}
\item 2\s\ with both minors, any strength, but not suitable for immediate 3\M\ or 4\s\ response.  Opener rebids 2\nt\ with a minimum or the better minor with a maximum.  If responder rebids 3\c\ over 2\nt, it is pass/correct.  Later bids of 3\M\ by responder are splinters and by inference either offers to play 3\nt\ or slam tries.  A jump to 4m shows 6-5 in the minors.
\item 2\h\ or 2\nt\+, showing a game invite or better with 5\+ spades or hearts respectively.  2\h\ forces 2\s, and the sequences are then symmetric:

\begin{tabular}{ll}
2\nt&balanced-ish invite\\
3m  &natural, invitational\\
3\M &slammish with 4\+\ cards in the corresponding minor\\
3\nt&choice of games\\
4\M &mild slam try with a good suit\\
4\x, $\x \ne \M$&splinter (even if \x\ = \s, I guess) with a good suit\\
4\nt&balanced-ish slam invite\\
5\nt&choice of slams
\end{tabular}
\end{itemize}

\section{1\nt--2\d/2\h: transfer (often 4)}

Opener usually accepts the transfer to 2\h\ or 2\s.  With both majors,
responder transfers to hearts and rebids 2\s\ (one-round force).  The
continuations are shown below.  Most importantly, responder's third
bid if at the 3 level is another transfer, allowing responder to
finish hand description.

\begin{figure}[ht]
\begin{tabular}{lllll}
2\nt&&&&minimum, no four-card major.\\
    &&&&Responder's 3\c\ then shows 55 in the majors, other bids are as after 3\c.\\
3\c &&&&any maximum\\
    &3\d&&&5\+\ \h, 4\+\ \s.  Forces 3\h.\\
    &   &3\h&\p&Possible if opener bid 2\nt.\\
    &   &   &3\s&55 majors, forcing\\
    &   &   &3\nt&choice of games\\
    &   &   &4m&cuebid self-agreeing \h\\
    &   &   &4\h&nonforcing slam try\\
    &3\h&&&4 \h, 5\+\ \s.  Forces 3\s.\\
    &   &3\s&\p&Possible if opener bid 2\nt\\
    &   &   &3\nt&choice of games\\
    &   &   &4m&cuebid self-agreeing \s\\
    &   &   &4\s&nonforcing slam try\\
    &3\s&&&?\\
    &3\nt&&&exactly 44 majors, choice of games\\
    &4\M&&&to play\\
3\M &&&&minimum with 4 \M.  Over this 3\nt\ should be a suggestion to play,\\
    &&&&since opener cannot move over a frivolous 3\nt.
\end{tabular}
\caption{Continuations after 1\nt--2\d--2\h--2\s\ (both majors, \inv\+).}
\end{figure}

If responder does not have both majors, the rebid is 2\nt\ or higher,
parallel with 1\nt--2\c, except that responder is showing a 4-card
major, a possible 5\+-card minor, or a bad long major.

XXX superaccept continuations?

\section{1\nt--2\s: range ask or \c\ (not \inv)}

This response shows either a balanced game or slam invite, or a weak
or \gf\+\ hand with clubs.  Opener rebids 2\nt\ with a minimum and
3\c\ with a maximum.  Responder rebids 3\c\ or passes to play there,
bids 3\d\ for CONFIT, 3\nt\ or maybe 6\nt\ or 7\nt\ to play, or
anything else showing clubs.  3\M\ is a splinter.  4\d is presumably
keycard, so no way to show a diamond splinter, sorry.

\section{1\nt--2\nt: weak doubleton ask}

Asks opener to bid a suit of doubleton Jx or worse, or 3\nt\ without
one.  Typically responder has an invitational hand with a long minor,
and is planning to pass or bid 3\nt\ or maybe 4m (nonforcing) or 5m.

\section{1\nt--3\c: \d\ (not \inv)}

Transfer to \d, either to play, or to rebid a splinter 3\M\ or make a
nonforcing slam try via 3\nt.  A 4\h\ rebid is presumably keycard, and
4\d\ a forcing slam try.

\section{1\nt--3\d: invite with long major}

As usual for multi-style bids, 3\h\ declines the invitation if
responder has \h, but might be accepting if responder has \s.  In
other words, 3\h\ is pass/correct, and 3\s\ is nonforcing.  3\nt,
4\c\ and 4\d\ all accept both majors: 4\d\ forces responder to bid the
long major, 4\c\ forces responder to transfer to the major, and
3\nt\ gives responder the option, with the obvious continuations.

\section{1\nt--3M: M splinter (31)(54)}

Shows the values for 3\nt, not necessarily slammish.  Opener's rebids
are natural and nonforcing, except raising the splinter suit, which
asks for responder to bid 5 of a minor.

\section{Higher responses}

\begin{tabular}{ll}
3\nt&to play\\
4\c&Gerber\\
4\d&Texas transfer (then 4\s\ is RKC 1430)\\
4\h&Texas transfer (then 4\nt\ is RKC 1430)\\
4\s&55 minors, choice of minor-suit games\\
4\nt&balanced slam invite, usually 33(43)\\
5m&to play
\end{tabular}
